    ...
    SubShader
    {
        ...
        CGPROGRAM
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
                ...
                float outline =
                    tex2D(_MainTex, IN.texcoord + float2(-1,  0) * _MainTex_TexelSize.xy).a + 
                    tex2D(_MainTex, IN.texcoord + float2( 1,  0) * _MainTex_TexelSize.xy).a + 
                    tex2D(_MainTex, IN.texcoord + float2( 0, -1) * _MainTex_TexelSize.xy).a + 
                    tex2D(_MainTex, IN.texcoord + float2( 0,  1) * _MainTex_TexelSize.xy).a;

                if (outline > 0.5 && color.a < 0.5) return float4((0.0).rrr, 1.0);
